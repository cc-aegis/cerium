...
impl Expression {
    pub(crate) fn compile_into(self, target: Operand) -> Result<(Vec<Instruction>, Option<CeriumType>), CompilerError> {
        match self {
            Expression::Scope(_, scope) => scope.compile_into(target),
...
impl Scope {
    fn compile_into(self, target: Operand) -> Result<(Vec<Instruction>, Option<CeriumType>), CompilerError> {
        let mut result = Vec::new();
        for instruction in self.instructions {
            result.append(&mut instruction.compile()?.0);
        }
        let return_type = if let Some(value) = self.value {
            let (mut inst, return_type) = value.compile_into(target)?;
